      ...
      <ProjectList>
        {data?.map((project, index) => (
          <Project
            ...
            onClick={(e) => {
              e.preventDefault()
              animatePageOut(`/${project.slug}`, router, pathname)
            }}
          >
            <InnerProjectContent>
              <DisplayText>{project.name}</DisplayText>
              <MediumText>{project.thumbnail.description}</MediumText>
            </InnerProjectContent>
            <ImageWrapper className="zoom">
              <CustomImage
                src={project.thumbnail.image.url}
                alt={project.thumbnail.image.alternaitiveText}
                speed={0.5}
